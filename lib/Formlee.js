...
const Formlee = ({ formData, onSubmit, isSubmitted, defaultValues, setIsSubmitted, inputTemplates, }) => {
    const { values, handleChange, updateValues } = useForm_1.useForm(defaultValues);
    const { errors, validate } = useValidator_1.useValidator(formData.fields, values);
    ...
    React.useEffect(() => {
        updateValues(defaultValues !== null && defaultValues !== void 0 ? defaultValues : {});
    }, [defaultValues]);
    React.useEffect(() => {
        if (isSubmitted) {
            handleSubmit();
